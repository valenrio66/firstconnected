...
func (ac *AuthController) SignInUser(ctx *gin.Context) {
	...
	// Generate Tokens
	access_token, err := utils.CreateToken(configs.AccessTokenExpiresIn, user.ID, configs.AccessTokenPrivateKey)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": err.Error()})
		return
	}
	...
	ctx.SetCookie("access_token", access_token, configs.AccessTokenMaxAge*60, "/", "localhost", false, true)
	ctx.SetCookie("refresh_token", refresh_token, configs.RefreshTokenMaxAge*60, "/", "localhost", false, true)
	ctx.SetCookie("logged_in", "true", configs.AccessTokenMaxAge*60, "/", "localhost", false, false)
